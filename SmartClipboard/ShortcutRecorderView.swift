...
struct ShortcutRecorderView: View {
    ...
    private func keyString(for keyCode: Int) -> String {
        // Simple mapping for common keys, or fallback to key number
        switch keyCode {
    ...
    private func startRecording() {
        ...
        // Use a local monitor to capture the next valid key press
        eventMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { event in
            // Must have at least one modifier to be a valid global shortcut (usually)
            // Or just allow anything for now but usually global hotkeys need modifiers.
    ...
    private func clearShortcut() {
        ...
        GlobalHotkeyManager.shared.install() // Re-install base hotkeys
    }
}
